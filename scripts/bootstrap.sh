...
#      the library's flutter-v0.11.0 tag — see README.md's "How the
#      submodule is pinned").
#   2. Fetches the release-published, checksum-verified mobile binaries via
#      the plugin's own fetch script (v0.11.0 native artifacts).
#
...
echo "==> Fetching released mobile binaries (mdviewer v0.11.0, checksum-verified)..."
if ! MDVIEWER_VERSION="v0.11.0" "$fetch_script"; then
  ...
  echo "       v0.11.0 entry. See README.md for which ref is pinned and why." >&2
  exit 1
fi
